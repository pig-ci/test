...
# --- 參數設定：改為讀取環境變數 (Render 配置) ---
# 這些變數會在 Render 介面中設定，以實現動態控制
...
DELAY_SECONDS = float(os.environ.get("DELAY", 0.1)) # 預設值

# 模擬 User-Agent (可以考慮擴展為隨機清單)
HEADERS = {
...
def run_flickr_test(num_requests, delay):
    ...
    return "<br>".join(log) # 使用 <br> 讓網頁輸出換行
